 ...
 * as leaf nodes. A parse tree is a hierarchical collection of AST
 * nodes (or, more correctly, of instances of concrete subclassses such
 * as VarDecl, ForStmt, and AssignExpr).
 *
 * Location: Each node maintains its lexical location (line and columns
 * in file), that location can be NULL for those nodes that don't care/
 * use locations. The location is typcially set by the node construc-
 * tor. The location is used to provide the context when reporting se-
 * mantic errors.
 *
 ...
 * node one level up in the parse tree.  The parent is not set in the
 ...
 * children into the parent node and then set up links in both direc-
 * tions. The parent link is typically not used during parsing, but is
 * more important in later phases.
 *
 * Semantic analysis: For pp3 you are adding "Check" behavior to the
 * AST node classes. Your semantic analyzer should do an inorder walk
 * on the parse tree, and when visiting each node, verify the particu-
 * lar semantic rules that apply to that construct.
 */
